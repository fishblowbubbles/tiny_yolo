...
class YOLO(nn.Module):
    ...
    def forward(self, x):
        ...
        proposal = (x.view(batch_size, len(self.anchors), self.n_classes + 5,
                           grid_w, grid_h).permute(0, 1, 3, 4, 2).contiguous())
        # ???
        offset_x = (torch.arange(grid_w).repeat(grid_h, 1).view(
            [1, 1, grid_h, grid_w]).type(torch.FloatTensor))
        offset_y = (torch.arange(grid_h).repeat(grid_w, 1).t().view(
            [1, 1, grid_h, grid_w]).type(torch.FloatTensor))
        # ???
        ...
        # x = P_w * d_x(P) + P_x
        output[..., 0] = (torch.sigmoid(proposal[..., 0]).data +
                          offset_x) * stride_w
        # y = P_h * d_y(P) + P_y
        output[..., 1] = (torch.sigmoid(proposal[..., 1]).data +
                          offset_y) * stride_h
        # w = P_w * exp(d_w(P))
        output[..., 2] = (torch.exp(proposal[..., 2]).data *
                          anchor_w) * stride_w
        # h = P_h * exp(d_h(P))
        output[..., 3] = (torch.exp(proposal[..., 3]).data *
                          anchor_h) * stride_h
        # confidence
